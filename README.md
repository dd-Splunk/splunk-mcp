...
Internal Claude Enterprise business-case materials are local-only and gitignored under **`demo-slides/claude-enterprise-*`**. The optional **`make marp-bizcase-*`** and **`make marp-onepager-pdf`** targets check for those local sources before exporting.
