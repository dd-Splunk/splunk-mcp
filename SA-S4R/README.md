...
**Workshop modes** — prefer Splunk MCP (see [MCP-TOOLS.md](../docs/s4r/MCP-TOOLS.md)):

| Action | MCP tool |
| ------ | -------- |
| Check mode | **`SA-S4R_query_nk_demo_state`** |
| Infrastructure (default) | **`SA-S4R_apply_nk_demo_state`** (`mode=infrastructure`) |
| Active threat (NK geo) | **`SA-S4R_apply_nk_demo_state`** (`mode=threat`); validate with **`SA-S4R_validate_nk_attack_traffic`** |

MCP toggles reload Eventgen — **no `make restart`**. Shell fallback (`scripts/toggle-s4r-attack-nk.sh` via `make s4r-attack-nk-*`): **`make restart`** after enable/disable, wait ~2 minutes. Validation SPL and per-agent expectations: [docs/s4r/SA-S4R-APP.md](../docs/s4r/SA-S4R-APP.md).
