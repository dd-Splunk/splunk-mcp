...
- **Local run recipe (gitignored files):** `.env` (Path B: your generated `SPLUNK_PASSWORD`/`SPLUNK_MCP_PASSWORD`, real `SPLUNKBASE_*`, `SPLUNK_IMAGE=splunk/splunk:9.3.2`) plus a `docker-compose.override.yml` that adds `cgroup: host`, sets a working `SPLUNK_APPS_URL`, and points the `so1-var` volume at the ext4 mount (`driver_opts: {type: none, o: bind, device: /mnt/splunkdb}`). Then `./scripts/compose-up.sh` (or `make up`). Wipe `so1-etc` + re-mkfs `/mnt/splunkdb` between clean runs (a stale `so1-etc` keeps the old admin password → 401, and skips app installs).
