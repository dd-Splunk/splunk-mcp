...
- **`splunk/splunk:latest` (10.4.x) crashes on startup** here: SIGABRT in `ContainerInfo::available_memory_size_in_bytes` (Splunk can't parse cgroup-v2 memory in this nested env). **Pin `SPLUNK_IMAGE=splunk/splunk:9.3.2`** (verified working) and run the container with **`cgroup: host`** (needed so the memory controller/`memory.max` is visible; the default private cgroup ns is threaded with no `memory` controller).
- **Splunkbase creds gate `make up`.** `SPLUNKBASE_USER`/`SPLUNKBASE_PASS` (a real splunk.com account) are needed to download the MCP Server app (7931) + Eventgen (1924) from `SPLUNK_APPS_URL`. With placeholder creds the base image's Ansible aborts (`Invalid Splunkbase credentials`). Without them you can only run the **base engine** (no `/services/mcp`) by clearing `SPLUNK_APPS_URL`, `SPLUNKBASE_USERNAME`, `SPLUNKBASE_PASSWORD`.
- **Local run without full `make up`** (gitignored files, base engine only): create `.env` (Path B, from the `SPLUNK_PASSWORD`/`SPLUNK_MCP_PASSWORD` you generate, `SPLUNKBASE_*` empty, `SPLUNK_IMAGE=splunk/splunk:9.3.2`) plus a `docker-compose.override.yml` that adds `cgroup: host`, empties `SPLUNK_APPS_URL`/`SPLUNKBASE_USERNAME`/`SPLUNKBASE_PASSWORD`, and points the `so1-var` volume at the ext4 mount (`driver_opts: {type: none, o: bind, device: /mnt/splunkdb}`). Then `docker compose up -d so1` and wait for `healthy`. This is how the dev env was verified: ingest via `POST /services/receivers/simple`, search via `POST /services/search/jobs/export` (REST/SPL on `:8089`; Web on `:8000`).
