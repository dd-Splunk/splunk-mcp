...
### `make up` vs `make init`

Secrets are **not generated by this repo**; they are **read from 1Password** (via `op://` in `tpl.env`) or **set by hand** in `.env` (Path B). The two flows below are both valid; pick one.

- **`make up` when there is no `.env` file:** The Makefile runs `op run --env-file=tpl.env -- docker compose up -d`. That **resolves** your vault references into the **environment of that command only**. The Makefile path does **not** create a `.env` file. You can repeat `make up` on every install as long as `op` is available—nothing extra is “stale” on disk from that step.
- **`make init` (optional):** Also uses `op run` + `tpl.env`, but then **`materialize-env.sh` writes a resolved `.env` file** on disk (plaintext, git-ignored, restrictive `umask`). After that, **`make up` uses plain `docker compose`**; Compose loads **`.env`** and **`op` is not required** for that later `make up` (you still have secrets in a local file, which is a **tradeoff** for convenience or CI).
- **For a typical local demo, skip `make init`:** `tpl.env` + `make up` is enough. Use **`make init`** when you need a file-based `.env` (e.g. automation) or you want to run `make up` without `op` after materializing—see [CONFIGURATION.md](CONFIGURATION.md#generating-env-optional).
