...
- **`.env`**: Optional; generated by **`make init`** (`op run` + **`scripts/materialize-env.sh`**) and **git-ignored** when present. Alternatively, **`make up`** may pass secrets via **`op run`** without creating `.env` (fewer secrets on disk).
