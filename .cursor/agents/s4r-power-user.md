...
5. Each Task prompt: read `.cursor/agents/s4r-[team].md` + `docs/S4R-SPL-CATALOG.md` § [team]; run via `splunk_run_query`; return that team's summary only. Specialists are **background** subagents — launch in parallel without blocking on each one.
